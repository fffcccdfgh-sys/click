...
Pipeline::Pipeline(const std::string &detModelDir,
  ...
  LOGD("Pipeline init done dictSize=%d configSize=%d",
       static_cast<int>(charactor_dict_.size()),
       static_cast<int>(Config_.size()));
}
...
std::string Pipeline::ProcessBitmap(const cv::Mat &rgbaImage) {
  LOGD("ProcessBitmap start image=%dx%d channels=%d", rgbaImage.cols,
       rgbaImage.rows, rgbaImage.channels());
  if (rgbaImage.empty()) {
    return "";
  }
  ...
  for (int i = boxes.size() - 1; i >= 0; i--) {
    ...
    if (!res.first.empty()) {
      rec_text.push_back(res.first);
      LOGD("PaddleOCR line %d %s, %f", i, res.first.c_str(), res.second);
    }
  }
