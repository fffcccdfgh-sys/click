...
object OcrAreaMapper {
    ...
    fun mapScreenAreaToCaptureAreaAllowingRotation(
        ...
        if (isWithinCaptureBounds(left, top, right, bottom, captureWidth, captureHeight)) {
            return rotated
        }

        // Percent-based script areas should still scale from the reported screen
        // size into the captured frame. Fall back to capture-oriented coordinates
        // when the normal mapping is impossible.
    ...
    private fun isWithinCaptureBounds(
        left: Int,
        top: Int,
        right: Int,
        bottom: Int,
