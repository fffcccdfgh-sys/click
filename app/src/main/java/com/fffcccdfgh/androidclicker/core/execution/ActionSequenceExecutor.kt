...
import com.fffcccdfgh.androidclicker.core.program.ProgramActionExecutor
...
object ActionSequenceExecutor {
    ...
    fun execute(
        ...
        onBlocked: (() -> Unit)? = null,
        extraLuaGlobalRegistrars: List<ProgramLuaGlobalRegistrar> = emptyList()
    ) {
        if (isRunning) {
        ...
        val service = AccessibilityServiceStatus.connectedService()
        if (service == null) {
            val messageRes = if (AccessibilityServiceStatus.isEnabled(context)) {
                R.string.tap_service_not_connected
            } else {
                R.string.tap_service_disabled
            }
    ...
    private suspend fun executeLoop(
        ...
        maxLoops: Int,
        loopGap: Long,
        extraLuaGlobalRegistrars: List<ProgramLuaGlobalRegistrar>
    ) {
        var loop = 0
        while (coroutineContext.isActive) {
            ...
            while (i < sequence.size && coroutineContext.isActive) {
                ...
                if (repeatCount == 0) {
                    ...
                    while (coroutineContext.isActive) {
                        val executed = executeSingleAction(
                            service,
                            action,
                            extraLuaGlobalRegistrars
                        )
